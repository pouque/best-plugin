...
def idfun(a): return a       # a -> a
def head(xs): return xs[0]   # list a -> a
def last(xs): return xs[-1]  # list a -> a
def tail(xs): return xs[1:]  # list a -> a
def single(x): return [x]    # a -> list a
def zip_self(a):  # list a -> list
    return zip(a, tail(a) + single(last(a)))


# returns True with p probality
def probality_choice(p):  # (n : int) -> {cond : n > 0 ∧ n ≤ 1} -> bool
    return random.random() < p


def same_case(ch1, ch2):  # string -> string -> string
    if ch1.istitle() and head(ch2).isupper():
        return ch1.upper()
    else:
        return ch1
...
class PluginState:
    ...
    def get_level_by_name(cls, name):  # string -> ()
        ...
        if finded is not None:
            ...
            raise IndexError(u"level “%s” not found" % name)

    @classmethod
    def set_level(cls, lvl):  # int -> ()
        if lvl < len(cls.modes):
            cls.current_level = lvl
        else:
            raise IndexError("mode level out of range (maximum is %i)" %
                             len(cls.modes))
...
class HuettaMode(PluginMode):
    def get_random_word(self):  # {HuettaMode} -> () -> string
        words = [u'блядь', u'сука', u'нахуй', u'пидор', u'чмо', u'хер',
                 ...
                 u'геегомик', u'гомогомик', u'говноед', u'говноглот',
                 u'мудак', u'кретин', u'еблан', u'шизик', u'говносос',
                 u'полудурок', u'пидорас', u'пидорасина', u'мразотина',
                 u'мразь', u'шизик', u'гуталин', u'шизогей', u'ебанько',
                 u'хохол', u'тварь', u'тварина', u'ватник', u'недоносок',
                 u'нигрилла', u'гомонигра', u'ниггер', u'дырявый',
                 u'таракан', u'русофоб', u'соплежуй', u'спермоглот']
        word = words[random.randint(0, len(words) - 1)]
        if probality_choice(self.word_insert_probality):
            return word
        else:
            return word.upper()
    ...
    def to_huetta(self, args):  # {HuettaMode} -> string -> string
        if probality_choice(self.attack_probality) and \
           (not args.startswith("/")):
...
weechat.hook_command("tr", "vot takoueu translit", "[text]",
                     "description-huiction", "list", "tr_command", "")
...
weechat.hook_command("translit", "make your text great again", "[text]",
                     "description-huiction", "list", "translit_command", "")
...
weechat.hook_command("toggle_translit", "perekljutschaet lutschschiij plagin",
                     "", "", "", "toggle_translit", "")
...
weechat.hook_command("translit_info", "eenoframcija",
                     "", "", "", "translit_info", "")
...
weechat.hook_command("random_toggle", "random toggle",
                     "", "", "", "random_toggle", "")
